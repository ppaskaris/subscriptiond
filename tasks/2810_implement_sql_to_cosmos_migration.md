...
Implement the designed test-server migration as a safe, resumable, secret-conscious command that writes final Cosmos document shapes.
...
- Reuse application mappers/projection-sizing utilities so migrated and newly written documents have identical shapes and invariants.
...
- Switching the deployed test-server configuration.
...
- The local unit, LocalDB, and Cosmos suites pass sequentially with both provider suites opted in.
